<?php
/*
Plugin Name: WP nyroModal Integration
Plugin URI: http://www.myhtmlworld.com
Description: Integrate nyroModal jquery plugin into your blogs. Open Images and Native Galleries on the same window without touching the html code.
Version: 1
Author: Sunil SK
...
function get_image_send_to_editor_same($id, $alt, $title, $align, $url='', $rel = false, $size, $same_window) {
	...
	if ($same_window =='yes'){
		...
		$html = '<a class="nyroModal" rel="same-post-'.$_REQUEST['post_id'].'" title = "'.$title.'" href="' . clean_url($url) ."\">$html</a>";
...
function add_tickbox_integration_css() {
	...
	echo '<link rel="stylesheet" type="text/css" href="' . get_bloginfo('wpurl') . '/wp-content/plugins/wp-nyro/nyroModal.css" />';
}
...
/*****************************************************************************
					 Adding class name to galleries
******************************************************************************/
	...
	// If no caption is defined, set the title and alt attributes to title
		...
		$output .= "
			<{$icontag} class='gallery-icon'>
					...
					<a href=\"$a_img\" title=\"$title\" class=\"nyroModal\" rel=\"gallery-$post->ID\">
...
function nyroModal_css_int() {
	wp_enqueue_style('nyroModal', get_bloginfo('wpurl') . '/wp-content/plugins/wp-nyro/nyroModal.css');
	wp_print_styles(array('nyroModal'));
}

add_action('wp_head', 'nyroModal_css_int');
wp_enqueue_script('jquery');


function nyroModal_js_int() {

	$tpi_header =  "\n<!--  WP nyroModal Integration -->\n";
	$tpi_header .= "<script type=\"text/javascript\"";
	$tpi_header .= " src=\"".get_bloginfo('wpurl') . "/wp-content/plugins/wp-nyro/jquery.nyroModal.js\">";
	$tpi_header .= "</script>\n";
...
add_action('wp_head', 'nyroModal_js_int');
